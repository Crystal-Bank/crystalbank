...
describe CrystalBank::Domains::Roles::Creation::Events::Accepted do
  it "can be initialized" do
    event = Test::Role::Events::Creation::Accepted.new.create
    ...
    event.header.event_handle.should eq("role.creation.accepted")
    event.body.comment.should eq("test comment")
  end

  it "can be initialized from json" do
    object = Test::Role::Events::Creation::Accepted.new
    event = object.create
    h = event.header

    json_body = JSON.parse(object.json_string)

    e = Roles::Creation::Events::Accepted.new(h, json_body)
    b = e.body.as(Roles::Creation::Events::Accepted::Body)
  ...
  it "can serialize to json" do
    ...
    b = event.body.as(Roles::Creation::Events::Accepted::Body)
  ...
  it "can serialize and deserialize from json" do
    ...
    e = Roles::Creation::Events::Accepted.new(h, JSON.parse(json_body))

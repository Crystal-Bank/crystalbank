...
private def make_opening_context(scope_id : UUID) : CrystalBank::Api::Context
...
private def seed_opening_customer(scope_id : UUID) : UUID
  customer_id = UUID.v7
...
private def make_opening_request(customer_ids : Array(UUID)) : Accounts::Api::Requests::OpeningRequest
...
describe CrystalBank::Domains::Accounts::Opening::Commands::Request do
  ...
  it "raises when a customer belongs to a different scope" do
    other_scope = UUID.v7
    customer_id = seed_opening_customer(other_scope)
  ...
  it "creates an account opening request when all customer IDs are valid and in scope" do
    customer_id = seed_opening_customer(scope_id)

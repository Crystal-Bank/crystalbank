...
describe CrystalBank::Domains::VirtualAccounts::Api::VirtualAccounts do
  ...
  describe "GET /accounts/:id/virtual - scope filtering" do
    it "returns the virtual subaccount when scope matches" do
      ...
      results = VirtualAccounts::Queries::VirtualAccounts.new.list(parent_account_id, context, cursor: nil, limit: 100)
    ...
    it "returns no results for a different parent account" do
      ...
      results = VirtualAccounts::Queries::VirtualAccounts.new.list(UUID.v7, context, cursor: nil, limit: 100)
      results.should be_empty
    end
  end
end

...
private def sepa_command(aggregate_id : UUID, r : Payments::Sepa::CreditTransfers::Api::Requests::CreditTransferRequest, user_id : UUID, scope_id : UUID) : Payments::Sepa::CreditTransfers::Initiation::Commands::Request
  Payments::Sepa::CreditTransfers::Initiation::Commands::Request.new(aggregate_id: aggregate_id, r: r, actor_id: user_id, scope_id: scope_id)
end

describe CrystalBank::Domains::Payments::Sepa::CreditTransfers::Initiation::Commands::RequestHandler do
  before_all do
  ...
  it "creates a SEPA Credit Transfer and an approval when accounts are valid" do
    ...
    payment_id = UUID.v7
    result = Payments::Sepa::CreditTransfers::Initiation::Commands::RequestHandler.new.handle(sepa_command(payment_id, request, user_id, scope_id))
  ...
  it "raises when debtor account is not open" do
    ...
    expect_raises(CrystalBank::Exception::InvalidArgument, /not open/) do
      Payments::Sepa::CreditTransfers::Initiation::Commands::RequestHandler.new.handle(sepa_command(UUID.v7, request, UUID.v7, UUID.v7))
    end
  end

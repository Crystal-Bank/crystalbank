...
describe CrystalBank::Domains::ApiKeys::Generation::Events::Accepted do
  it "can be initialized" do
    event = Test::ApiKey::Events::Generation::Accepted.new.create
    ...
    event.header.event_handle.should eq("api_key.generation.accepted")
    event.body.comment.should eq("test comment")
  end

  it "can be initialized from json" do
    object = Test::ApiKey::Events::Generation::Accepted.new
    event = object.create
    h = event.header

    json_body = JSON.parse(object.json_string)

    e = ApiKeys::Generation::Events::Accepted.new(h, json_body)
    b = e.body.as(ApiKeys::Generation::Events::Accepted::Body)
  ...
  it "can serialize to json" do
    ...
    b = event.body.as(ApiKeys::Generation::Events::Accepted::Body)
  ...
  it "can serialize and deserialize from json" do
    ...
    e = ApiKeys::Generation::Events::Accepted.new(h, JSON.parse(json_body))

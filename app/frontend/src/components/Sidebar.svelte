<script>
  ...
  function toggleDropdown() {
    if (!showDropdown && scopeOptions.length === 0) fetchScopes()
...
<aside class="w-56 bg-zinc-900 flex flex-col flex-shrink-0">
  ...
  <div class="px-3 py-3 border-b border-zinc-800 relative">
    <!-- App branding -->
    ...
    <!-- Dropdown Menu -->
    {#if showDropdown}
      <div class="absolute left-3 right-3 z-50 bg-zinc-800 border border-zinc-700 rounded-md shadow-xl overflow-hidden" style="top: calc(100% - 0.25rem)">
        <!-- All Scopes option -->
        <button
          type="button"
          onclick={clearScope}
        ...
        >
          ...
          <span class="flex-1 text-sm text-zinc-200 truncate">All Scopes</span>
          {#if !auth.scope}
        ...
        {#if scopeOptions.length > 0}
          <div class="border-t border-zinc-700/60 my-0.5"></div>
          <div class="max-h-52 overflow-y-auto">
            {#each scopeOptions as s (s.id)}
              <button
                ...
                class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left transition-colors"
                style="background: {auth.scope === s.id ? 'rgba(255,255,255,0.08)' : 'transparent'}"
                onmouseenter={(e) => { e.currentTarget.style.background = 'rgba(255,255,255,0.08)' }}
                onmouseleave={(e) => { e.currentTarget.style.background = auth.scope === s.id ? 'rgba(255,255,255,0.08)' : 'transparent' }}
              >
                <div class="w-5 h-5 rounded bg-zinc-600 flex items-center justify-center flex-shrink-0 text-white font-bold" style="font-size:0.6rem">
                  {s.name ? s.name[0].toUpperCase() : '?'}
                </div>
                <div class="flex-1 min-w-0">
                  <div class="text-sm text-zinc-200 truncate">{s.name}</div>
                  <div class="text-xs text-zinc-500 truncate font-mono">{s.id.slice(0, 8)}…</div>
                </div>
                {#if auth.scope === s.id}
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.6)" stroke-width="2.5">
                    <path d="M20 6L9 17l-5-5"/>

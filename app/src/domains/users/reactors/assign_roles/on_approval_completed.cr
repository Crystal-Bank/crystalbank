    ...
    module AssignRoles
      class OnApprovalCompleted < ES::Reactor
        def call(event : Approvals::Collection::Events::Completed)
          ...
          # Only process if this approval is for a UserRolesAssignment request
          ...
          Users::AssignRoles::Commands::AcceptHandler.new(event_store: @event_store, event_handlers: @event_handlers).handle(
            Users::AssignRoles::Commands::Accept.new(aggregate_id: request_id, requestor_id: approval.state.requestor_id)
          )
        end
      end
    end

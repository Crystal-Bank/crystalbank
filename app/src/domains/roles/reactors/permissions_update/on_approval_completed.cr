module CrystalBank::Domains::Roles
  module Reactors
    module PermissionsUpdate
      class OnApprovalCompleted < ES::Reactor
        def call(event : Approvals::Collection::Events::Completed)
          approval = Approvals::Aggregate.new(event.header.aggregate_id, event_store: @event_store, event_handlers: @event_handlers)
          approval.hydrate

          # Only process if this approval is for a RolePermissionsUpdate
          return unless approval.state.source_aggregate_type == "RolePermissionsUpdate"

          update_request_id = approval.state.source_aggregate_id.as(UUID)

          Roles::PermissionsUpdate::Commands::AcceptHandler.new(event_store: @event_store, event_handlers: @event_handlers).handle(
            Roles::PermissionsUpdate::Commands::Accept.new(aggregate_id: update_request_id, requestor_id: approval.state.requestor_id)
          )
        end
      end
    end
  end
end

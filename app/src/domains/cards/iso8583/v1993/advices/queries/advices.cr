  ...
  module Queries
    class Advices
      ...
      def get(id : UUID, context : CrystalBank::Api::Context) : AdviceResult?
        @db.query_one?(
          "SELECT * FROM projections.iso8583_v1993_advices WHERE uuid = $1 AND scope_id = ANY($2)",
      ...
      def list(context : CrystalBank::Api::Context, cursor : Int32 = 0, limit : Int32 = 50) : Array(AdviceResult)
        @db.query_all(
          "SELECT * FROM projections.iso8583_v1993_advices WHERE scope_id = ANY($1) ORDER BY created_at DESC LIMIT $2 OFFSET $3",

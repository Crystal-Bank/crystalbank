  ...
  module Queries
    class Authorizations
      struct AuthorizationResult
        ...
        property account_id_1 : String?
        property account_id_2 : String?
      ...
      def get(id : UUID, context : CrystalBank::Api::Context) : AuthorizationResult?
        @db.query_one?(
          "SELECT * FROM projections.iso8583_v1993_authorizations WHERE uuid = $1 AND scope_id = ANY($2)",
      ...
      def list(context : CrystalBank::Api::Context, cursor : Int32 = 0, limit : Int32 = 50) : Array(AuthorizationResult)
        @db.query_all(
          "SELECT * FROM projections.iso8583_v1993_authorizations WHERE scope_id = ANY($1) ORDER BY created_at DESC LIMIT $2 OFFSET $3",

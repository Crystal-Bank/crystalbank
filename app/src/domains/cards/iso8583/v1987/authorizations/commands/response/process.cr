    ...
    module Commands
      class Process < ES::Command
        ...
        def call(authorization_id : UUID, msg : Shared::Message, actor_id : UUID) : Nil
          raise ArgumentError.new("Expected MTI 0110, got #{msg.mti}") unless msg.mti == "0110"
          ...
          if APPROVED_CODES.includes?(response_code)
            ...
            event = V1987::Authorizations::Response::Events::Approved.new(
            ...
            event = V1987::Authorizations::Response::Events::Declined.new(
              aggregate_id: authorization_id,
              actor_id: actor_id,
              command_handler: self.class.to_s,
              response_code: response_code,

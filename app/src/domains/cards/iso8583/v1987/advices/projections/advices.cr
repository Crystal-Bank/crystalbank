module CrystalBank::Domains::Cards::Iso8583::V1987::Advices
  module Projections
    class Advices < ES::Projection
      define_projection "projections.iso8583_v1987_advices", init: true do
      ...
      apply(V1987::Advices::Advice::Events::Received) do |event|
        @projection_database.transaction do |tx|
          tx.exec(<<-SQL, event.header.aggregate_id, event.stan, event.pan_masked,
            INSERT INTO projections.iso8583_v1987_advices
              (uuid, stan, pan_masked, amount, currency, terminal_id, merchant_id,
               original_authorization_id, scope_id, status, created_at, updated_at)
      ...
      apply(V1987::Advices::AdviceResponse::Events::Accepted) do |event|
        @projection_database.transaction do |tx|
          tx.exec(<<-SQL, event.header.aggregate_id, event.response_code)
            UPDATE projections.iso8583_v1987_advices
      ...
      apply(V1987::Advices::AdviceResponse::Events::Rejected) do |event|
        @projection_database.transaction do |tx|
          tx.exec(<<-SQL, event.header.aggregate_id, event.response_code)
            UPDATE projections.iso8583_v1987_advices

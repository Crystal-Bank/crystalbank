module CrystalBank::Domains::Accounts
  module UnblockingRequest
    class Aggregate < ES::Aggregate
      ...
      def apply(event : ::Accounts::UnblockingRequest::Events::Requested)
        @state.increase_version(event.header.aggregate_version)
        body = event.body.as(::Accounts::UnblockingRequest::Events::Requested::Body)
        @state.account_id = body.account_id
        @state.block_type = body.block_type
        @state.reason = body.reason
      end

      def apply(event : ::Accounts::UnblockingRequest::Events::Completed)
        @state.increase_version(event.header.aggregate_version)

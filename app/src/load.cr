# Load libs
require "pg"
require "yaml"
require "crystal-es"
require "crystal_iban"

# Load config
require "./config/load"

# Load shared resources
require "./shared/load"

# Load domains
require "./domains/account_management/accounts/load"
require "./domains/account_management/virtual_accounts/load"
require "./domains/api_keys/load"
require "./domains/approvals/load"
require "./domains/customers/load"
require "./domains/events/load"
require "./domains/roles/load"
require "./domains/scopes/load"
require "./domains/ledger/transactions/load"
require "./domains/payments/load"
require "./domains/users/load"
require "./domains/platform/load"
require "./domains/cards/load"
